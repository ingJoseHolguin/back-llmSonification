...
from config.constans import CHROMA_SETTINGS
from config.context_prompt import context_prompt
import os
import argparse

odel = os.environ.get("MODEL", "llama3.2:1b")
...
def response(query: str) -> str:
    ...
    llm = Ollama(
        model=model, callbacks=callbacks, temperature=0, base_url="http://ollama:11434"
    )
